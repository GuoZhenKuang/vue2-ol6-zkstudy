 ...
 * @LastEditTime: 2022-03-01 14:30:02
 * @LastEditors: 阿匡
 * @Description: 三维数据展示
 * @FilePath: \vue2-ol-zkstudy\src\components\cesiumDemos\Layer3dData\index.vue
 * 仅为学习使用
-->
<template>
  <div class="layer3dTree">
    <el-tree
      :data="data"
      show-checkbox
      default-expand-all
...
<script>
import execute3DMixin from "../Map3D/mixin/execute3DMixin";
export default {
  name: "layer2dControlData",
  mixins: [execute3DMixin],
  data() {
    return {
      data: [],
      defaultProps: {
  ...
  methods: {
    ...
    handleCheckChange(checkedNodes, checked) {
      if (!checkedNodes.children) {
        //不接收父节点的数据
        if (checkedNodes.is2dMap == false && checked == true) {
          //三维数据加载
